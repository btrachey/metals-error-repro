...
//import io.github.edadma. // try tab completion of this line
...
object Main extends App {
  ...
  val (sndfile, sfinfo) = open( // try buf.hover() on open() and get nothing back
    "sine.wav",
    SFM_WRITE,
    SFInfo(
      ...
      frames = SAMPLE_COUNT,
      channels = 2,
      format = FORMAT_WAV subtype FORMAT_PCM_24
    )
  )
  ...
  def sample(k: Int): Int =
    if (sfinfo.channels == 1) (AMPLITUDE * sin(LEFT_FREQ * 2 * k * Pi)).toInt
      ...
      if ((k & 1) == 0) // left channel indexes are even
        (AMPLITUDE * sin(
          LEFT_FREQ * k * Pi
        )).toInt // k = 2*i where i is the right channel sample
      else
        (AMPLITUDE * sin(
          RIGHT_FREQ * (k - 1) * Pi
        )).toInt // k = 2*i + 1 where i is the left channel sample
    }
